...
export function absorbNewHotels(
  G: IG, chain: Chain, id: string, idsAbsorbed: Set<string> = new Set([id])) {
...
export function declareGameOver(G: IG, ctx: Ctx, isGameOver: boolean) {
  if (isGameOver) {
    ...
    const chains = new Set<Chain>(G.hotels.flat().map(h => h.chain).filter(c => !!c));
...
export const MergersGame: Game<IG> = {
  ...
  phases: {
    ...
    'chooseChainToMergePhase': {
      ...
      onBegin: (G: IG, ctx: Ctx) => autosetChainToMerge(G),
    },
    ...
    'mergerPhase': {
      ...
      onBegin: (G: IG, ctx: Ctx) => awardBonuses(G, G.chainToMerge),
